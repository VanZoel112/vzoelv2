...
class AutonomousBotCreator:
    ...
    def get_created_bots_report(self) -> str:
        """Generate report of created bots"""
        
        if not self.created_bots["bots"]:
            return f"{emoji('merah')} No bots created yet"
        
        report_lines = [
            f"{emoji('utama')} {bold('AUTONOMOUS BOTS REPORT')}",
            "",
            f"{emoji('centang')} {bold('Summary:')}",
            ...
            "",
            f"{emoji('loading')} {bold('Created Bots:')}"
        ]
        
        for i, bot in enumerate(self.created_bots["bots"][-10:], 1):  # Show last 10
            status_emoji = emoji('centang') if bot.get('status') == 'active' else emoji('merah')
        ...
        report_lines.append(f"{italic('Generated by Vzoel Autonomous Bot Creator')}")
...
class PremiumEmojiMapper:
    ...
    def get_emoji_report(self) -> str:
        """Generate emoji mapping report"""
        ...
        report_lines = [
            f"{emoji('utama')} {bold('PREMIUM EMOJI MAPPING REPORT')}",
            "",
            f"{emoji('centang')} {bold('Summary:')}",
            f"• Total Emojis: {bold(str(total_emojis))}",
            f"• Premium Emojis: {bold(str(premium_emojis))}",
            f"• Last Updated: {bold(self.emoji_mapping.get('last_updated', 'Never')[:19] if self.emoji_mapping.get('last_updated') else 'Never')}",
            "",
            f"{emoji('loading')} {bold('Available Emojis:')}"
        ]
        
        for key, data in list(self.emoji_mapping["emojis"].items())[:10]:  # Show first 10
            status = "Premium" if data.get("premium", True) else "Standard"
            report_lines.append(f"• {bold(key)}: {data['alt']} ({status})")
        
        if total_emojis > 10:
            report_lines.append(f"... and {bold(str(total_emojis - 10))} more emojis")
        
        report_lines.extend([
            "",
            f"{emoji('aktif')} {bold('Usage Example:')}",
            f"```python",
            f"emoji_mapper.send_premium_message(",
            f"    chat_id=chat_id,",
            f"    text=\"Hello {{fire}} World {{star}}!\",",
            f"    emoji_mapping={{\"fire\": \"vzoel_fire\", \"star\": \"vzoel_star\"}}",
            f")```",
            "",
            f"{italic('Generated by Vzoel Premium Emoji Mapper')}"
        ])
